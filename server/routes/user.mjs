    ...
    if (validateToken(token)) {
      ...
      if (imgData && units) {
        result = await User.updateOne(
          ...
          { unit_pref: units, profile_img: imgData }
        );
  ...
  try {
    ...
    if (validateToken(token)) {
      ...
      const userDetails = await User.findOne(
        { _id: tokenData.userId },
        "username unit_pref profile_img liked_recipes"
      );

      // userDetails is either a document or null – no .length
      if (!userDetails) {
...
router.get("/likes", async (req, res) => {
  try {
    ...
    // Respond with a simple array of IDs
    res.status(200).json({ likes: user.liked_recipes || [] });
  } catch (err) {
